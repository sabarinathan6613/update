...
export async function getSampleStationDirectConfig() {
  const supabase = getSupabaseClient();
  const defaultConfig = {
    lump: {
      sampleEquipmentIds: [],
      shiftIdEquipmentId: '',
      shiftCumulativeEquipmentId: '',
      stockpileEquipmentId: ''
    },
    fines: {
      sampleEquipmentIds: [],
      shiftIdEquipmentId: '',
  ...
  try {
    const { data, error } = await supabase
      .from('email_configuration')
      .select('password')
      .eq('id', 'sample_station_direct_config')
      .maybeSingle();
    if (!error && data && data.password) {
      const parsed = JSON.parse(data.password);
      if (parsed && typeof parsed === 'object') {
        return {
          lump: { ...defaultConfig.lump, ...(parsed.lump || {}) },
          fines: { ...defaultConfig.fines, ...(parsed.fines || {}) }
        };
      }
    }
  } catch (e) {
    console.error('[getSampleStationDirectConfig] Failed:', e);
  }
  return defaultConfig;
}

export async function saveSampleStationDirectConfig(config) {
  const supabase = getSupabaseClient();
  if (!supabase) throw new Error('Supabase client is not initialized');
  const { error } = await supabase
    .from('email_configuration')
    .upsert({
      id: 'sample_station_direct_config',
      host: 'sample_station_direct_config',
      port: 587,
      username: 'direct_config@skadomation.internal',
      password: JSON.stringify(config),
      secure: false
    }, { onConflict: 'id' });
  if (error) {
    console.error('[saveSampleStationDirectConfig] Failed:', error);
